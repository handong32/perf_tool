...
static void print_interval(char *prefix, struct timespec *ts)
{
	...
	//FILE *tx, *rx;
	//long long int rxp, txp;
	//int ret;
	
/*	rx = fopen("/sys/class/net/enp4s0f1/statistics/rx_packets", "r");
	tx = fopen("/sys/class/net/enp4s0f1/statistics/tx_packets", "r");
	ret=fscanf(rx, "%lld", &rxp);
	ret=fscanf(tx, "%lld", &txp);
	fclose(rx);
	fclose(tx);
	
	///sys/class/net/enp4s0f1/statistics/tx_packets
	//printf("RX:%lld TX:%lld ret=%d\n", rxp, txp, ret);
	*/
...
static void print_counters(struct timespec *ts, int argc, const char **argv)
{
	...
	char cstates[6][10] = { "POLL", //residency=0, latency=0
				"C1", //residency=2, latency=2
				"C1E", //residency=20, latency=10
				"C3", //residency=211, latency=80
				"C6", //residency=345, latency=104
				"C7" //residency=345, latency=109
	};
